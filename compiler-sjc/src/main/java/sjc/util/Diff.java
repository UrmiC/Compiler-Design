/*
 * $Log: Diff.java,v $
 * Revision 1.1  2012/08/29 20:50:37  robby
 * An alternative milestone 2.
 *
 * Revision 1.2  2012/08/29 19:33:27  robby
 * Removed warnings.
 *
 * Revision 1.1  2009/05/30 00:31:46  ericmercer
 * Modified the classpath to ignore the ANTLR generated test harness
 * Moved the test utils into the sjc.test.util package
 * Removed ANTLR generated files and added comments to grammar file
 *
 * Revision 1.2  2008/09/04 20:56:34  robby
 * Updated to ANTLR 3.1 and ANTLRWorks 1.2.
 *
 * Revision 1.1  2008/08/17 10:20:20  robby
 * Initial commit.
 *
...
public class Diff {
   ...
   * Each change represents one place where some lines are deleted and some are
   * inserted.
   * 
  ...
  public static class change {
    ...
    /**
     * Cons an additional entry onto the front of an edit script OLD. LINE0 and
     * LINE1 are the first affected lines in the two files (origin 0). DELETED
     * is the number of lines deleted here from file 0. INSERTED is the number
     * of lines inserted here in file 1.
     * 
     * If DELETED is 0 then LINE0 is the number of the line before which the
     * insertion was done; vice versa for INSERTED and LINE1.
     */
  ...
  class file_data {
    ...
    /**
     * Vector, indexed by line number, containing an equivalence code for each
     * line. It is this vector that is actually compared with that of another
     * file to generate differences.
     */
    ...
    /**
     * Array, indexed by real origin-1 line number, containing true for a line
     * that is an insertion or a deletion. The results of comparison are stored
     * here.
     */
    boolean[] changed_flag;

    @SuppressWarnings({ "rawtypes", "unchecked" })
    file_data(final Object[] data, final Hashtable h) {
      ...
      for (int i = 0; i < data.length; ++i) {
        final Integer ir = (Integer) h.get(data[i]);
        if (ir == null) {
          h.put(data[i], new Integer(this.equivs[i] = Diff.this.equiv_max++));
        } else {
     ...
     * discarded lines) into real line numbers; this is how the actual
     ...
     * possible.
     * 
     * We do something when a run of changed lines include a blank line at one
     * end and have an excluded blank line at the other. We are free to choose
     * which blank line is included. `compareseq' always chooses the one at the
     * beginning, but usually it is cleaner to consider the following blank line
   ...
   * heuristic, for files with a constant small density of changes, the
  ...
  /**
   * Prepare to find differences between two arrays. Each element of the arrays
   * is translated to an "equivalence number" based on the result of
  ...
  public Diff(final Object[] a, final Object[] b) {
    final Hashtable h = new Hashtable(a.length + b.length);
    this.filevec[0] = new file_data(a, h);
    this.filevec[1] = new file_data(b, h);
  }

  /**
   * Compare in detail contiguous subsequences of the two files which are known,
   * as a whole, to match each other.
   * 
   * The results are recorded in the vectors filevec[N].changed_flag, by storing
   ...
   * doing a breadth-first search through the space of edit-sequence. When the
   * two searches meet, we have found the midpoint of the shortest edit
   * sequence.
   * 
   ...
   * number of deleted lines (counting only lines before the midpoint). The edit
   * cost is stored into COST; this is the total number of lines inserted or
   * deleted (counting only lines before the midpoint).
   * 
   * This function assumes that the first lines of the specified portions of the
   * two files do not match, and likewise that the last lines do not match. The
   * caller must trim matching lines from the beginning and end of the portions
   * it is going to specify.
   * 
   * Note that if we return the "wrong" diagonal value, or if the value of bdiag
   * at that diagonal is "wrong", the worst this can do is cause suboptimal diff
  ...
  private int diag(final int xoff, final int xlim, final int yoff,
    ...
    for (int c = 1;; ++c) {
      ...
      for (d = fmax; d >= fmin; d -= 2) {
        int x, y, oldx;
        final int tlo = fd[this.fdiagoff + d - 1], thi = fd[this.fdiagoff + d
            + 1];
      ...
      for (d = bmax; d >= bmin; d -= 2) {
        int x, y, oldx;
        final int tlo = bd[this.bdiagoff + d - 1], thi = bd[this.bdiagoff + d
            + 1];
      ...
      if ((c > 200) && big_snake && this.heuristic) {
        ...
        for (d = fmax; d >= fmin; d -= 2) {
          final int dd = d - fmid;
          if ((fd[this.fdiagoff + d] - xoff) * 2 - dd > 12 * (c + (dd > 0 ? dd
              : -dd))) {
        ...
        for (d = bmax; d >= bmin; d -= 2) {
          final int dd = d - bmid;
          if ((xlim - bd[this.bdiagoff + d]) * 2 + dd > 12 * (c + (dd > 0 ? dd
              : -dd))) {
  ...
  /**
   * Get the results of comparison as an edit script. The script is described by
   * a list of changes. The standard ScriptBuilder implementations provide for
   * forward and reverse edit scripts. Alternate implementations could, for
   * instance, list common elements instead of differences.
   * 
   * @param bld
  ...
  public change diff(final ScriptBuilder bld) {
    ...
    final int diags = this.filevec[0].nondiscarded_lines
  ...
  public final change diff_2(final boolean reverse) {
    return diff(reverse ? Diff.reverseScript : Diff.forwardScript);
  }
