...
bool returnBoolean(bool b) {
	...
	i = 0 ;
	void* RU = !!((!c) && (false || true) || (i > 2 && i >= -1) || 3 == i && (+4 != i) || 3 < 5 && ~1 <= -+~2);
	return RU;
...
int main(char* str) {
	...
	m_array = newArray(vm,"struct ManyTypes*",&m_array ,i * sizeof (struct ManyTypes*), NULL) ;
	 checkArray(m_array,0, "m_array", i);
	checkNotNull(&m_array[0]);
	m_array[0] = m ;
	...
	m->i = 0 ;
	 checkArray(m_array,(checkArray(i_array,(checkNotNull(&(m->i))? m->i : m->i), "i_array", 5) && checkNotNull(&i_array[(checkNotNull(&(m->i))? m->i : m->i)])? i_array[(checkNotNull(&(m->i))? m->i : m->i)] : i_array[(checkNotNull(&(m->i))? m->i : m->i)]), "m_array", i);
	checkNotNull(&m_array[(checkArray(i_array,(checkNotNull(&(m->i))? m->i : m->i), "i_array", 5) && checkNotNull(&i_array[(checkNotNull(&(m->i))? m->i : m->i)])? i_array[(checkNotNull(&(m->i))? m->i : m->i)] : i_array[(checkNotNull(&(m->i))? m->i : m->i)])]);
	...
	for (
	i = 2 , 
	i = j  ; j < (+i << (checkLeftShift(+i,4)? 4 : 4)) && j == i ; j++, i--) {
	}
	...
	b_array = newArray(vm,"bool",&b_array , 2 * sizeof (bool), makeList(vm,&b_array, 2,true )) ;
	 checkArray(i_array,returnInt(), "i_array", 5);
	checkNotNull(&i_array[returnInt()]);
